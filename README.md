...
## Livro Caixa (divisão de funcionários)

Nova aba no Admin com três partes:
- **Funcionários**: cadastro livre (nome, função, pontos), fácil de adicionar ou remover a qualquer momento.
- **Divisão do dia**: escolha a data, marque quem trabalhou naquele dia — o sistema já sabe o total da taxa de serviço (10%) daquele dia (somado automaticamente dos fechamentos) e divide proporcionalmente aos pontos de cada funcionário selecionado. Clique em "Salvar divisão do dia" para guardar.
- **Histórico e Excel**: veja o resumo do mês (por dia e por funcionário) e clique em "Exportar Excel" para baixar um `.xlsx` com três abas (por dia, total por funcionário, e detalhado) — tudo gerado direto no navegador, sem precisar de nenhum serviço externo.
